...
```{r growth-model, fig.cap='Simulations from the growth model, compared to exponential and logistic growth for two levels of resource surplus $X$ over the same time span. $X$ acts as a carrying capacity. When $X$ is low, the potential for growth is low and the population approaches carrying capacity gradually similar to logistic growth. When $X$ is far above $N$ the population exponentially, with growth rates only declining shortly before carrying capacity is reached.', message = FALSE}
...
```{r functions, fig.cap='Functional forms for the spatial interaction models. A) Settlement-level variables influence the attractiveness of each settlement via a power function, with the parameter $\\alpha$ governing the importance of that variable. B) The costs of moving over space take the form of a negative exponential function, with the parameter $\\beta$ determining the steepness of the falloff of interaction with distance (higher values of $\\beta$ allow interaction to occur at farther distances).'}
...
ggsave('functions.png', width = 6, height = 6)
```
...
run_sim <- function(net, alpha1 = 1.15, alpha2 = 0, beta1 = 5, beta2 = 10, nu = .05){
  ...
  new_net <- net %>%
    ...
    # stop the simulation if all hexes have been at equilbirium for more than 50 years
...
```{r, eval = FALSE}
# By default, this and the following chunk are not run to save time, and instead
# results are precomputed and loaded in as separate data files. To run these chunks,
# set eval = TRUE.
...
param_sweep <- expand_grid(beta1 = c(5, 10, 15, 20),
 ...
 # filter(beta1 <= beta2) %>%
  mutate(sim = future_pmap(list(beta1, beta2, alpha1, alpha2), 
                           function(a,b,c,d) reduce(1:2000, ~run_sim(., beta1 = a, beta2 = b, alpha1 = c, alpha2 = d, nu = 0.05), 
                  .init = settlements), 
                  .progress = TRUE))

saveRDS(param_sweep, 'derived_data/param_sweep')
```
...
param_dat <- param_sweep %>%
  mutate(nodes = map(sim, as_tibble, 'nodes'),
         population = map_dbl(nodes, ~sum(.$population)),
         ...
         terminals_trade = map_dbl(sim, ~sum(nystuen_dacey(.)$terminal)),
         terminals_mig = map_dbl(sim, ~sum(nystuen_dacey(., mode = 'migrants')$terminal))) %>%
  select(-sim, -nodes, -prob_vect)

node_dat <- param_sweep %>%
  mutate(nodes = map(sim, as_tibble, 'nodes')) %>%
  select(-sim) %>%
  unnest(col = c(nodes)) %>%
  select(-food, -migrant_production, -id)

edge_dat <- param_sweep %>%
  mutate(edges = map(sim, ~activate(., 'edges') %>% filter(trade_flow > 1 | migrant_flow > 1) %>% as_tibble)) %>%
  select(-sim)

saveRDS(nd_graphs, 'derived-data/nd_graphs')
saveRDS(param_dat, 'derived-data/param_dat')
saveRDS(node_dat, 'derived-data/node_dat')
saveRDS(edge_dat, 'derived-data/edge_dat')
```
...
```{r}
nd_graphs <- readRDS('derived-data/nd_graphs')
param_dat <- readRDS('derived-data/param_dat')
node_dat <- readRDS('derived-data/node_dat')
edge_dat <- readRDS('derived-data/edge_dat')
```
...
```{r beta1, fig.cap = "The role of distance deterrence of trade in settlement size and spacing. A) Equilibrium settlement patterns and populations for different levels of $\\beta_1$, with $\\beta_2 = 20, \\alpha_1 = 1.15, \\alpha_2 = 0$. B) Same as A, but for each hinterland an edge is drawn connecting it to the settlement to which the majority of its resources flow. Trade flow is measured in units of food to support 1 person per year.", warning = FALSE}
# Note: this figure might display oddly when knit under the elsarticle template because of its strict image sizing.
# the correctly size image is saved to a file instead
a <- node_dat %>%
  filter(alpha1 == 1.15, alpha2 == 0, beta2 == 20) %>%
...
#ggsave('beta1.png', height = 6, width = 9)
```

```{r beta2, fig.cap='Settlement population (A) and trade flows (B) at equilibrium for different levels of $\\beta_2$$, the distance deterrence for migration, with $\\beta_1 = 5, \\alpha_1 = 1.15, \\alpha_2 = 0$.', warning = FALSE}
# Note: this figure might display oddly when knit under the elsarticle template because of its strict image sizing.
# the correctly size image is saved to a file instead
a <- node_dat %>%
   filter(alpha2 == 0, beta1 == 5, alpha1 == 1.15) %>%
  group_by(alpha1, beta2)%>%
  arrange(population) %>%
    filter(population >= 1) %>%
  ungroup %>%
  mutate(beta2 = as.factor(beta2),
         beta2 = fct_relabel(beta2, ~paste('beta[2] == ', .)))

nd_graphs %>%
  filter(alpha1 == 1.15, alpha2 == 0, beta1 == 5) %>%
  mutate(nd = map(trade, as_tibble, 'edges')) %>%
  select(beta1:nu, nd) %>%
  unnest(col = c(nd)) %>%
  tbl_graph(nodes = as_tibble(paths, 'nodes'), edges = .) %E>%
  ...
  scale_edge_color_viridis(name = 'Trade flow') +
    geom_point(data = a, aes(x=x, y=y, size = population)) +
  scale_size_area(name = 'Population') +
  facet_edges(~beta2, labeller = 'label_parsed') +
  coord_sf(datum = NA) +
  theme_void()

#ggsave('beta2.png', height = 6, width = 9)
```
...
node_dat %>%
  ...
  ungroup %>%
      mutate(beta2 = as.factor(beta2),
         beta2 = fct_relabel(beta2, ~paste('beta[2] == ', .)),
         alpha1 = as.factor(alpha1),
         alpha1 = fct_relabel(alpha1, ~paste('alpha[1] == ', .))) %>%
  ggplot() +
  geom_sf(data = hinterlands, fill = NA, color = 'grey65') +
  ...
  coord_sf(datum = NA) +
  theme_void() +
  facet_grid(rows = vars(alpha1), cols = vars(beta2), labeller = 'label_parsed')

ggsave('beta2alpha1.png', width = 8, height = 8)
```
...
```{r beta2alpha2, fig.cap='Interaction between the distance deterrence for migrants ($\\beta_2$, columns) and the returns to attractiveness for per capita welfare ($\\alpha_2$, rows), when moving food to settlements is difficult and migrants make decisions based on both settlement population size and per capita welfare ($\\beta_1 = 5$ and $\\alpha_1 = 1.15$).'}
node_dat %>%
    filter(beta1 == 5, alpha1 == 1.15) %>%
  ...
  arrange(population) %>%
    ungroup %>%
      mutate(beta2 = as.factor(beta2),
         beta2 = fct_relabel(beta2, ~paste('beta[2] == ', .)),
         alpha2 = as.factor(alpha2),
         alpha2 = fct_relabel(alpha2, ~paste('alpha[2] == ', .))) %>%
  ggplot() +
  geom_sf(data = hinterlands, fill = NA, color = 'grey65') +
  geom_point(aes(x, y, size = population))+
  scale_size_area(name = 'Population') +
